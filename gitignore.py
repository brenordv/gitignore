...
def _parse_args():
    ...
    if qty_args < 2:
        print("You must inform the target folder.")
        exit(-1)
    if qty_args > 2:
        print("You need to pass only one argument: the target folder.")
        exit(-1)
    ...
    if not path.is_dir():
        print("The target folder is not a directory.")
        exit(-1)
...
def _extract_gitignore_data(config_item: dict):
    ...
    if len(gitignore_data) > 0:
        print("Using cached gitignore data...")
        return gitignore_data

    print("Fetching gitignore data...")
    url = config_item.get('url')
    if url is None:
        print(f"Invalid config item: {config_item}")
        return []

    response = requests.get(url)

    if not response.ok:
        print(f"Error to get gitignore data from {url}. Status code: {response.status_code}")
        return []

    gitignore_data = response.text.splitlines()

    print(f"Gitignore data fetched successfully. Caching {len(gitignore_data)} lines...")
...
def _dump_gitignore_data(gitignore_data: list, target_gitignore):
    if target_gitignore.exists():
        print("The .gitignore already exists. Merging with the new data...")
    ...
    print(f"Writing {len(sorted_clean_data)} lines to .gitignore...")
